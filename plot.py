...
# If None use all models found in logs
# DATASET_ORDER = None
DATASET_ORDER = ['goodreads']

# Range of budgets to include
BUDGETS = range(1, 6)
...
# Plot a line for the mean recall of GT explainer (when available)
# else, include GT as an explainer
GROUND_TRUTH_LINE = True
...
def main():
    ...
    assert len(datasets) > 0, "No datasets included, check DATASET_ORDER"
    assert len(models) > 0, "No datasets included, check MODEL_ORDER"
    assert len(explainers) > 0, "No explainers included, check EXPLAINER_ORDER"
    ...
    if args.plot_type == 'bias':
        ...
        for i, dataset in enumerate(datasets):
            for j, model in enumerate(models):
                ...
                ax = sns.barplot(data=data, x='Model Bias', y='Accuracy',
                        hue='Region', hue_order=['R', 'Not R'], # dodge=False,
                        ax=axes[i, j])
        ...
        for i, dataset in enumerate(datasets):
            for j, model in enumerate(models):
                ...
                hue_order = explainers.copy()

                # Get palette, but replace with set colors
    ...
    if n_subplots == 1:
        ...
        fig.subplots_adjust(top=0.95, bottom=0.05, hspace=0.3, wspace=0.2)
